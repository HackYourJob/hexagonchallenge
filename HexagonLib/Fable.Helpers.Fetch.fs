...
let fetchAsync (url:string, init: RequestProperties list) : Async<Response> = 
...
let fetchAs<'T> (url:string, init: RequestProperties list) : Async<'T> = async {
...
let private ( ++ ) (a:'a list) (b:'a list) : 'a list = failwith "JS Only"
...
let postRecord<'T> (url,record:'T, properties: RequestProperties list) : Async<Response> =
    let props = 
